...
& $Cli compile --fqbn 'esp32:esp32:esp32:FlashMode=dio,FlashFreq=80,FlashSize=4M,PartitionScheme=default,CPUFreq=240,PSRAM=disabled' --build-path $Build --output-dir $Build $Sketch
...
$Merged = Join-Path $Out 'vrchat-cyd-pager-v1.0.0.bin'
...
(Get-FileHash $Merged -Algorithm SHA256).Hash.ToLowerInvariant() | Set-Content -NoNewline (Join-Path $Out 'vrchat-cyd-pager-v1.0.0.sha256')
